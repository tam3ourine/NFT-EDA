...
```{r}
g <- ggplot(punktest, aes(Hair))
g + geom_bar(na.rm = TRUE)
```


```{r}
g <- ggplot(punktest, aes(Eyes))
g + geom_bar(na.rm = FALSE)


punktest$Eyes[] <- lapply(punktest$Eyes, as.character)
str(punktest)


is.list(punktest$Eyes)



ggplot(data=punktest, aes(x=Eyes)) + 
geom_bar()
...
class(mydata$neighborhood)
class(punktest$Eyes)

ggplot(punktest,
       aes(x = Eyes, # x is neighborhood
           fill = Eyes)) + # fill is neighborhood
  geom_bar()
```
